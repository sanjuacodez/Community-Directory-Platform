...
export default function CreateFamilyPage() {
  ...
  const[f,setF]=useState({community_id:'',name:'',house_name:'',address:'',family_admin_id:''});const[error,setError]=useState('');const[loading,setLoading]=useState(false);
  ...
  return(<div className="mx-auto max-w-lg space-y-6"><h1 className="text-2xl font-bold">Create Family</h1><form onSubmit={submit} className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6">
    ...
    <div><label className="block text-sm font-medium">Family Admin (optional)</label><select value={f.family_admin_id} onChange={e=>s('family_admin_id',e.target.value)} className="input"><option value="">None</option>{members.filter(m=>!f.community_id||m.community_id===f.community_id).map(m=><option key={m.id} value={m.id}>{m.first_name} {m.last_name}</option>)}</select><p className="text-xs text-zinc-400 mt-1">Select a member to assign as admin. They'll manage this family.</p></div>
