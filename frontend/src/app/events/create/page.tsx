...
export default function CreateEventPage() {
  ...
  const[f,setF]=useState({community_id:'',title:'',description:'',event_date:'',location:'',image:''});const[error,setError]=useState('');const[loading,setLoading]=useState(false);
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setError('');setLoading(true);
    try{const{error:err}=await supabase.from('events').insert(Object.fromEntries(Object.entries(f).filter(([_,v])=>v)));if(err)throw new Error(err.message);router.push('/events');}
    catch(err:any){setError(err.message)}finally{setLoading(false)}};
  return(<div className="mx-auto max-w-lg space-y-6"><h1 className="text-2xl font-bold">New Event</h1><form onSubmit={submit} className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6">
    ...
    <div><label className="block text-sm font-medium">Date</label><input type="date" value={f.event_date} onChange={e=>setF({...f,event_date:e.target.value})} className="input" required/></div>
    ...
    <div><label className="block text-sm font-medium">Image URL</label><input value={f.image} onChange={e=>setF({...f,image:e.target.value})} className="input"/></div>
