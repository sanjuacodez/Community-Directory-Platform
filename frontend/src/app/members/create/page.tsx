...
export default function CreateMemberPage() {
  ...
  const[f,setF]=useState<any>({community_id:'',family_id:'',first_name:'',last_name:'',gender:'',visibility:'community_only',is_deceased:false});
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setError('');setLoading(true);
    try{const body:any={...f};Object.keys(body).forEach(k=>{if(body[k]===''&&k!=='is_deceased')delete body[k];});
  ...
  return(<div className="mx-auto max-w-lg space-y-6"><h1 className="text-2xl font-bold">Add Member</h1><form onSubmit={submit} className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6">
    ...
    <div><label className="block text-sm font-medium">Family*</label><select value={f.family_id} onChange={e=>s('family_id',e.target.value)} className="input" required><option value="">Select</option>{families.filter(ff=>!f.community_id||ff.community_id===f.community_id).map(ff=><option key={ff.id} value={ff.id}>{ff.name}</option>)}</select></div>
