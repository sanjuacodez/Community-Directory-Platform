  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div className="card" style={{ marginBottom: '1rem', padding: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.25rem', marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid var(--color-border)' }}>
          {tabs.map(t => (
            <button key={t.mode} onClick={() => setView(t.mode)} className={`btn btn-sm ${view === t.mode ? 'btn-primary' : 'btn-ghost'}`}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {members.map(m => (
            <div key={m.id} className="card" style={{ padding: '1rem' }}>
              ...
              <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', margin: 0 }}>{m.profession && <>{m.profession} · </>}{m.family?.name} · {m.community?.name}</p>
          ...
          {members.map(m => (
            <div key={m.id} className="card" style={{ padding: '0.75rem 1rem' }}>
              <div className="flex items-center justify-between">
                ...
                <span className="badge">{m.family?.name}</span>
              </div>
            </div>
          ))}
