...
export default function EditCommunityPage() {
  ...
  const [logo, setLogo] = useState(''); const [adminId, setAdminId] = useState('');
  ...
  useEffect(() => {
    ...
    supabase.from('communities').select('*').eq('id', p.id).single().then(({ data }) => {
      if (data) { setName(data.name); setSlug(data.slug); setLogo(data.logo ?? ''); setAdminId(data.admin_id ?? ''); setInit(false); }
    });
  }, [p.id]);
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setError(''); setLoading(true);
    try { const { error: err } = await supabase.from('communities').update({ name, slug, logo: logo || null, admin_id: adminId || null }).eq('id', p.id); if (err) throw new Error(err.message); router.push('/communities'); }
    catch (err: unknown) { setError(err instanceof Error ? err.message : 'Update failed'); } finally { setLoading(false); } };

  return (
    <div className="mx-auto max-w-lg space-y-6"><h1 className="text-2xl font-bold">Edit Community</h1>
      <form onSubmit={submit} className="space-y-4 rounded-xl border border-zinc-200 bg-white p-6">
        <div><label className="block text-sm font-medium">Name</label><input value={name} onChange={e => { setName(e.target.value); setSlug(generateSlug(e.target.value)); }} className="input mt-1" required /></div>
        ...
        <div><label className="block text-sm font-medium">Logo URL</label><input value={logo} onChange={e => setLogo(e.target.value)} className="input mt-1" /></div>
        <div><label className="block text-sm font-medium">Community Admin</label><select value={adminId} onChange={e => setAdminId(e.target.value)} className="input"><option value="">None</option>{members.map(m => <option key={m.id} value={m.id}>{m.first_name} {m.last_name}</option>)}</select><p className="text-xs text-zinc-400 mt-1">This person can manage the community, families, and members.</p></div>
